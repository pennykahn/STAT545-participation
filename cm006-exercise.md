...
```
## Error in library(FILLTHISIN): there is no package called 'FILLTHISIN'
```
...
```r
select(gapminder, FILLTHISIN)
```

```
## Error in select(gapminder, FILLTHISIN): could not find function "select"
```
